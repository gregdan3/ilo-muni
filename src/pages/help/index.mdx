...
> confusing, that would be why! This notice is dated 2024-12-16. If you're
...
While it looks like there is a mess of too many options, there are categories in
the dropdown which indicate their importance to you ("simple", "useful", and
"weird"), and there only four different types of scale in the first place:
**Linear**, **Logarithmic**, **Minmax**, and **Other**.

If you're not interested in the details, then stick to the simple category and
you'll be fine! Otherwise, read on.

### Linear

The linear scales are going to be the most familiar and obvious. They include
the [absolute](#absolute), [relative](#relative), and [cumulative](#cumulative)
scales, and in each, you have a series of exact numbers which are graphed over
time without changing the left axis or the magnitude of the numbers.

### Logarithmic

For example, the **Relative Log scale** is the natural logarithm of the
...
The **relative scale** is the default scale, and when graphing hits on it, it
shows you what **percentage of all words** are the searched term in each time
period on the graph. This also applies to phrases, for which you would interpret
as follows: "What portion of all words are **any one word** from this phrase?"
This is to say, the number of times a phrase appears can stand in for any of the
words in the phrase, from a math standpoint.
...
I recommend the [Relative Minmax scale](#minmax-scales) for cases like these,
...
to 2024-12-16, does not use monthly buckets of data. Instead, it uses 4-weekly
...
As of 2024-12-16, I have switched back to monthly buckets.
...
The **Window Avg** smoother is the default smoother, and the most recommended.
It averages
...
preserving the occurrence of large peaks, but still smears them forward in time
rather than

#### Gaussian

The **Gaussian** smoother makes highly smooth and presentable curves, and does a
better job of preserving peaks than job
...
spike in usage due to a word game being played, which I try to identify and omit
(since the goal is to measure "real" toki pona).
