...
[**mi kama sona e toki pona**](/?query=mi+kama+sona+e+toki+pona). Note: The more
words are in a term, the fewer times it is likely to appear- so don't be
...
[**toki, pona, toki pona**](/?query=toki,+pona,+toki+pona). Often, different
terms will have very different amounts of use, resulting in it being hard to
read more than the top one or two graphs. You can address that with
...
You can search for **multiple similar terms** by adding a **wildcard** `*`
anywhere in your search except the start, including in the middle of a word.
This will find the **ten most popular terms** matching your search and graph
them.

Note that the number of words in the terms your search finds depends on how many
"words" are in your search term. You can insert extra wildcards after a space to
search for longer terms.

<details>
  <summary>**Examples**</summary>

- Find the most popular prepositional phrases for "tawa":
  [**tawa \***](/?query=tawa+*)
- Observe the disparity in the top 1-2 uses of a word versus the rest:
  [**sona \***](/?query=sona+*), [**ona \***](/?query=ona+*)
- Find anything that starts with "at": [**at\***](/?query=at*)
- Find words starting with p and ending with a: [**p\*a**](/?query=p*a)
- Find phrases where the first word is "tawa" and the second starts with "w":
  [**tawa w\***](/?query=tawa+w*)
- Find three word phrases beginning with "la": [**la \* \***](/?query=la+*+*)
...
- Do the same with
  [UCSUR text](https://www.kreativekorp.com/ucsur/charts/sitelen.html)
  specifically: [**toki + 󱥬**](/?query=toki+%2B+󱥬)
...
- If you're viewing the **authors** or **hits per author** field, addition is
  disabled. This is because authors are sets, rather than integers. Read more in
  the [section on Authors](#authors).
- You can only use wildcards in one term of a search which uses addition. This
  limitation is necessary, because otherwise a search like **toki \* + musi \***
  would result in attempting to graph one hundred terms.
...
- Examine the use of a word while omitting conflicting but unrelated terms, like
  in [**san - kekan san**](/?query=san+-+kekan+san) or
  [**usawi - jan usawi**](/?query=usawi+-+jan+usawi)

</details>

**Notes**:

- If you're viewing the **authors** or **hits per author** field, subtraction is
  disabled. This is because authors are sets, rather than integers. Read more in
  the [section on Authors](#authors).
- You can only use wildcards in one term of a search which uses subtraction.
  This limitation is necessary, because otherwise a search like **toki \* -
  musi \*** would result in attempting to graph one hundred terms.

### Attributes

You can search for words and terms **in specific contexts** by appending an
underscore `_` and then any of the following attributes:

- **start**: Count only terms at the start of a sentence:
  [**taso_start**](/?query=taso_start)
- **end**: Count only terms at the end of a sentence:
  [**anu seme_end**](/?query=anu+seme_end)
- **inner**: Count only terms inside the sentence, neither at the
  start nor end: [**nasa_inner**](/?query=nasa_inner)
- **full**: Count only terms that are the entire sentence, at both the start
  and end: [**toki_full**](/?query=toki_full)
- **short**: Count only terms in "short" sentences, with fewer than 4 words:
  [**toki pona_short**](/?query=toki+pona_short)
- **long**: Count only terms in "long" sentences, with 4 or more words:
  [**kama pona_long**](/?query=kama+pona_long)

<details>
  <summary>**Examples**</summary>

- See the most common continuations after a "mi" sentence:
  [**mi \*\_start**](/?query=mi+*_start)
- See how sentences end when "la" is the second to last word:
  [**la \*\_end**](/?query=la+*_end)
- Combine with addition and subtraction to get more specific categories:
  [**wawa - wawa_full**](/?query=wawa+-+wawa_full)
...
<details>
  <summary>**Obsolete: Minimum Sentence Length**</summary>

**NOTE**: Minimum Sentence Length is no longer available in the current version
of ilo Muni. This section is kept for those studying older ilo Muni databases.
...
rendering every power of ten at the same scale. This means that the visual
...
[which you can see here](/?query=toki%2C+selo&scale=abs). On the logarithmic
scale though, the trend line of the less common word
[becomes visible](/?query=toki%2C+selo&scale=logabs), letting you see that it
still trends the same way as the much more popular word!
...
The **minmax scales** squash all of the data on the graph into the same range of
0 to 1, letting you directly compare the trend of a graph entirely regardless of
its magnitude. This scaling is achieved by finding the minimum and maximum
points of a line, then subtracting the minimum from every datapoint and dividing
each result by (max - min).
...
[**ona, li**](/?query=ona%2C+li&scale=normrel) trend similarly with use,
regardless of the fact that they have wildly different magnitudes of usage.
...
With this data, we can ask the following reasonable questions:
...
their percentages are measured against unrelated totals derived from unrelated
groups: "percentage of all words" is not comparable to "percentage of all
phrases of length 2", and percentages of each cannot be meaningfully added or
subtracted. Doing math as though they are comparable will create confusing and
incorrect results as we observed.
...
> What the y-axis shows is this: **of all the bigrams** [terms of length 2]
> contained in our sample of books written in English and published in the
> United States, **what percentage of them** are "nursery school" or "child
> care"?
...
recommend the [Relative Log scale](#logarithmic) or
[Relative Minmax scale](#minmax) for cases like these, which can help to
identify words which trend in the same way no matter their magnitude.
...
pona** from the 4 authors of **toki** and get 2 authors. But in the above
example, we know that there were 4 authors who said **toki**, and that only one
of those authors had said **toki pona**, so the correct result should be 3
authors.
...
This was fixed on August 13th 2024; now, smoothing does not occur until the
first non-zero element of a graph.
...
By default, **Gaussian** is set, and I don't recommend changing it from the
...
locations and relative sizes of peaks and troughs than either Window Average or
Exponential.
...
[phantom peaks](/?query=suno+pi+toki+pona&smoothing=7&smoother=cwin) with
sufficiently high smoothing.
...
[phantom peaks](/?query=suno+pi+toki+pona&smoothing=7&smoother=med) as Window
Average
...
[phrases with 2 or more words](#relative) to create a consistent interpretation
across the dataset. However, these are not the only interpretation challenges.
Most of the remaining bias is in where and how the data is collected, which I
describe the specifics of below.
...
[**tawa pona - kama pona**](/?query=tawa+pona+-+kama+pona), you'll get a graph
which is mostly negative. This means the phrase "kama pona" is more common than
the phrase "tawa pona", probably because the community is very welcoming!
...
### Why do hits and authors drop so much between Feb and Aug 2023?
