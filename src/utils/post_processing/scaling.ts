...
const logarithmicRelativeScale: RelativeScaler = (rows: Row[], totals: Row[]) =>
    relativeScale(logarithmicAbsoluteScale(rows), logarithmicAbsoluteScale(totals));
...
const absoluteDerivativeScale: AbsoluteScaler = (rows: Row[]) =>
    rows.map((row, i) => {
        const diff = i > 0
            ? row.occurrences - rows[i - 1].occurrences
            : 0;
...
const relativeSecondDerivativeScale: RelativeScaler = (rows: Row[], totals: Row[]) =>
